...
const concDataAccessor = (result) => {
  ...
  const newData = data.concentrations.map(
    ({ base_id: bId, conc_id: cId, ...rest }) => ({
      ...data.base[bId],
      ...rest,
  ...
  console.log(
    data.concentrations
      .map(({ base_id: bId, conc_id: cId, ...rest }) => ({
        ...data.base[bId],
        ...rest,
        program_college: programCollegeLookup[data.base[bId].program]
          ? programCollegeLookup[data.base[bId].program]
      ...
      .filter((el) => el.college !== el.program_college),
  );
    ...
    filterLists: (obj) => {
      ...
      const newLists = Object.fromEntries(
        Object.entries(lists).map((entry) =>
          entry[0] !== "minor_college"
            ? entry
            : [entry[0], new Set([...entry[1]].filter(Boolean))],
        ),
      );
